...
func (app *App) checkRecovery() {
	log := log.NewPrefixLogger(app.logger, log.RecoveryPrefix)
	if !app.IsRecoveryNeeded(app.config.Hostname) {
		return
	}
	if app.doesResetupFileExist() {
		log.Infof("resetup file exists, waiting for resetup to complete")
		return
	}
	...
	sstatus, err := localNode.GetReplicaStatus()
	if err != nil {
		log.Errorf("host %s failed to get slave status %v", localNode.Host(), err)
		return
	}
	if sstatus == nil {
		log.Info("waiting for manager to turn us to a new master")
		return
	}
	...
	if err != nil {
		log.Errorf("failed to get current master from dcs: %v", err)
		return
	}
	err = app.cluster.UpdateHostsInfo()
	if err != nil {
		log.Errorf("updating hosts info failed due: %s", err)
		return
	}
	masterNode := app.cluster.Get(master)
	mgtids, err := masterNode.GTIDExecutedParsed()
	if err != nil {
		log.Errorf("host %s failed to get master status %v", masterNode, err)
		return
	}

	log.Infof("master %s has GTIDs %s", master, mgtids)
	log.Infof("local node %s has GTIDs %s", localNode.Host(), sstatus.GetExecutedGtidSet())

	if isSlavePermanentlyLost(sstatus, mgtids) {
		rp, err := localNode.GetReplicaStatus()
		if err == nil {
			if rp.GetLastError() != "" {
				log.Errorf("local node %s has error: %s", localNode.Host(), rp.GetLastError())
			}
			if rp.GetLastIOError() != "" {
				log.Errorf("local node %s has IO error: %s", localNode.Host(), rp.GetLastIOError())
			}
		} else {
			log.Errorf("local node %s is NOT behind the master %s, need RESETUP", localNode.Host(), masterNode)
		}
		app.writeResetupFile("")
	} else {
		readOnly, _, err := localNode.IsReadOnly()
		if err != nil {
			log.Errorf("failed to check if host is read-only: %v", err)
			return
		}

		if !readOnly {
			log.Errorf("host is not read-only, we should wait for it...")
			return
		}

		log.Infof("local node %s is not ahead of master, recovery finished", localNode.Host())
		err = app.ClearRecovery(app.config.Hostname)
		if err != nil {
			log.Errorf("failed to clear recovery flag in zk: %v", err)
		}
	}
}
...
func (app *App) checkCrashRecovery() {
	...
	if err != nil {
		log.Errorf("failed to get local daemon state: %v", err)
		return
	}
	if !ds.StartTime.IsZero() {
		log.Debugf("daemon state: start time: %s", ds.StartTime)
	}
	if !ds.RecoveryTime.IsZero() {
		log.Debugf("daemon state: crash recovery time: %s", ds.RecoveryTime)
	}
	if !ds.CrashRecovery {
		return
	}
	...
	if sstatus == nil {
		log.Info("resetup after crash recovery may happen only on replicas")
		return
	}
	log.Errorf("local node %s is running after crash recovery %v, need RESETUP", localNode.Host(), ds.RecoveryTime)
	app.writeResetupFile("")
}
...
func (app *App) repairCascadeNode(node *mysql.Node, clusterState map[string]*NodeState, master string, cascadeTopology map[string]mysql.CascadeNodeConfiguration) {
	...
	app.logger.Infof("repair: checking cascade node %s. Replication=%v, cnc=%+v, slaveState=%+v", host, isReplicationRunning, cnc, state.String(app.config.ShowOnlyGTIDDiff))
